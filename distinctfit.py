...
fitplot(1,[[rst_p,rst_m]],[rst_fit],styles=[["bo"],["b-"]],title="Restricted Dataset",labels=[[None],[rst_str]])

gen_data = gdr2_cc_query(more="",loe=2,poe=8,dimmag=-1)
gen_p, gen_m = pmunpack(gen_data)
gen_fit, gen_str = logfit(gen_p,gen_m,name="General Dataset")
fitplot(2,[[gen_p,gen_m]],[gen_fit],title="General Dataset",labels=[[None],[gen_str]])
...
grm_fit, grm_str = logfit(grm_p,grm_m,name="General Dataset with Removals")
fitplot(3,[[grm_p,grm_m]],[grm_fit],title="General Dataset with Basic Removals",labels=[[None],[grm_str]])
...
ref_fit, ref_str = logfit(ref_p,ref_m,name="Refined Dataset",sig=50-ref_p) # Using a sketchy weighting method. Seems to work ok but no good reasoning behind it.
fitplot(4,[[ref_p,ref_m]],[ref_fit],title="Refined Dataset",labels=[[None],[ref_str]])
